...
import { 
  ...
  Language, Security, LocalShipping 
} from '@mui/icons-material';
import { motion, type Variants } from 'framer-motion'; 
import { useNavigate } from 'react-router-dom';

// --- 1. COMPONENTES MOTION CORREGIDOS ---
  ...
  return (
    <Box sx={{ bgcolor: '#fff', color: '#000', overflowX: 'hidden', minHeight: '100vh' }}>
      ...
      >
        <Toolbar sx={{ justifyContent: 'space-between', px: { xs: 2, md: 8 }, py: 1 }}>
          <Typography 
            ...
            sx={{ fontWeight: 900, letterSpacing: '-3px', cursor: 'pointer' }}
          >
            DENIM_LAB.
          </Typography>
          ...
          <Box sx={{ display: { xs: 'none', md: 'flex' }, gap: 5 }}>
            ...
            <Button onClick={() => scrollToSection('servicios')} sx={{ color: '#000', fontSize: '0.7rem', fontWeight: 700 }}>SERVICIOS</Button>
          </Box>
          ...
          <Grid container spacing={8} alignItems="center">
            <Grid size={{ xs: 12, sm: 5 }}>
              ...
              <Typography sx={{ color: '#666', mb: 4 }}>
                Esculpimos tu identidad con servicios diseñados para que tu denim sea eterno.
              </Typography>
            </Grid>
            ...
            <Grid size={{ xs: 12, sm: 7 }}>
              <Stack spacing={6}>
                {[
                  { icon: <LocalShipping />, title: 'LOGÍSTICA GLOBAL', desc: 'Envíos asegurados y tracking en tiempo real.' },
                  { icon: <Security />, title: 'SASTRERÍA VITALICIA', desc: 'Ajustes sin costo para siempre.' },
                  { icon: <Language />, title: 'SOSTENIBLE', desc: 'Procesos con ahorro de agua Denim_Lab.' }
                ].map((service, index) => (
                  <MotionBox 
                    key={index}
      ...
      <Box id="coleccion" sx={{ py: 10, px: { xs: 2, md: 5 } }}>
        <Typography variant="h6" sx={{ fontWeight: 900, mb: 6, textAlign: 'center', letterSpacing: 4 }}>NEW ARRIVALS</Typography>
